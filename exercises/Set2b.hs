...
oddFactorial x = if odd x && x>0 
                 then oddFactorial (x - 2) * x
                 else if x <= 0
                 then 1
...
countdown2 :: Integer -> String

countdown n = if n == 0
              then ""
              else if n == 1
              then "Ready! 1... Liftoff!"
              else "Ready! " ++ countdown2 n ++ "Liftoff!"

countdown2 0 = ""
countdown2 n = show n ++ "... " ++ countdown2 (n - 1)
...
smallestDivisor n = div2 n 2

div2 n k = if mod n k == 0
           then k
           else div2 n (1+k)
